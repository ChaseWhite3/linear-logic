...
Inductive Provable : Proof -> Assumptions -> Formula -> Prop :=
| P_L_Id    : forall A,
  Provable (Pr_L_Id) ((A_Linear A)::nil) A
| P_I_Id    : forall A,
  Provable (Pr_I_Id) ((A_Intuit A)::nil) A
...
| P_ImplEl  : forall (Gamma Delta : Assumptions) (A B : Formula),
  Provable Pr_Bad Gamma (F_Impl A B) -> 
  Provable Pr_Bad Delta A -> 
  Provable Pr_Bad (Gamma++Delta) B
| P_BothId  : forall (Gamma Delta: Assumptions) (A B : Formula),
  Provable Pr_Bad Gamma A -> 
  Provable Pr_Bad Delta B -> 
  Provable Pr_Bad (Gamma ++ Delta) (F_Both A B)
| P_BothEl  : forall (Gamma Delta : Assumptions) (A B C : Formula),
  Provable Pr_Bad Gamma (F_Both A B) ->
  Provable Pr_Bad (Delta ++ (A_Linear A)::(A_Linear B)::nil) C ->
  Provable Pr_Bad (Gamma++Delta) C
| P_ChooId  : forall (Gamma : Assumptions) (A B : Formula),
  Provable Pr_Bad Gamma A ->
  Provable Pr_Bad Gamma B ->
  Provable Pr_Bad Gamma (F_Choo A B)
...
| P_EithEl  : forall (Gamma Delta: Assumptions) (A B C: Formula),
  Provable Pr_Bad Gamma (F_Eith A B) ->
  Provable Pr_Bad (Delta ++ (A_Linear A)::nil) C ->
  Provable Pr_Bad (Delta ++(A_Linear B)::nil) C ->
  Provable Pr_Bad (Gamma ++ Delta) C.
Hint Constructors Provable.

Section all_cases.
 Variable all : Assumptions -> list (Proof * Formula).
 Hypothesis all_nil_eq:
  all nil = nil.
 Hint Rewrite all_nil_eq.
 Hypothesis all_sound:
  forall A p f,
   In (p,f) (all A) -> Provable p A f.
...
Lemma all_P_L_Id_sound:
 forall A p f,
  In (p,f) (all_P_L_Id A) -> Provable p A f.
Proof.
 induction A as [|a A]; simpl; intros p f; try tauto.
